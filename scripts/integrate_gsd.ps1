<#
.SYNOPSIS
    GSD-2 (Pi SDK) Integration (PowerShell)
#>
...
Write-LogStep "GSD-2 Integration (one-stop MCP config) - PowerShell"
...
# GSD-2 looks for .gsd/mcp.json or project-level .mcp.json
$gsdDir = Join-Path $targetDir ".gsd"
if (-not (Test-Path $gsdDir)) { New-Item -ItemType Directory -Path $gsdDir -Force | Out-Null }
$gsdMcpJson = Join-Path $gsdDir "mcp.json"
Backup-File $gsdMcpJson
...
$config | ConvertTo-Json -Depth 10 | Set-Content $gsdMcpJson
Write-LogOk "Wrote $gsdMcpJson"

# Also write to discoverable top-level .mcp.json
$topMcpJson = Join-Path $targetDir ".mcp.json"
Backup-File $topMcpJson
$config | ConvertTo-Json -Depth 10 | Set-Content $topMcpJson
Write-LogOk "Wrote discoverable .mcp.json to $targetDir"
